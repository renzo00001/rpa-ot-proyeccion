...
def Leer_archivo_txt(ruta_txt,solo_material=False) -> pl.DataFrame :
    ...
    df = df.drop(df.columns[1:4],df.columns[6],df.columns[8:12],df.columns[13],df.columns[17:19],df.columns[21:25],df.columns[26:30] , df.columns[31:42])
    df = df.rename({ df.columns[0]:'OT' ,  df.columns[1]:'clase_documento' ,  df.columns[2]:'fecha_creado' ,  df.columns[3]:'creado_por' ,  df.columns[4]:'CD',
                    ...
                    df.columns[10]:'fecha_entrega',df.columns[11]:'tiene_entrega',  df.columns[12]:'Cantidad_entregada' })
    ...
    df = df.with_columns( pl.col('OT').cast(pl.Int64),
                    ...
                    pl.when(pl.col('tiene_entrega')=='X').then( pl.lit('SI') ).otherwise(pl.lit('NO')).alias('tiene_entrega') )
    if solo_material:
